...
Based on the results on ACF and PACF, I found that there are seasonal autoregressive and moving average effects. So I ran the ARIMA model with changing order of p,d, q and seasonal orders. First, I ran the auto arima to get a rough estimate of my orders of seasonal ARIMA. Since the original series looks stationary, I do not need to consider the high order of differencing(**d**). The best fit ARIMA model with the lowest AIC is **ARIMA $(4, 1, 1) X (0, 1, 1)_{12}$ **

```{r, fig.width=5, fig.height=7,echo=FALSE}
m_acf <-  rasterGrob(as.raster(readPNG("./images/model_acf.png")), interpolate = FALSE)
m_pacf <-  rasterGrob(as.raster(readPNG("./images/model_pacf.png")), interpolate = FALSE)
grid.arrange(m_acf, m_pacf, ncol = 2)
```

After fitting the model, it is important to diagnostic the residuals of the ARIMA model. The blue line shows the 95% confidence interval under the null hypothesis of white noise. 
